...
#
#             if (i + 2) % 4 in pipe[tunnel[nr][nc]]: # 연결이 되어 있는지 확인
...
def DFS(r, c, time):
    ...
    for i in pipe[tunnel[r][c]]:
        ...
        if 0 <= nr < N and 0 <= nc < N and tunnel[nr][nc] and ((i+2)%4 in pipe[tunnel[nr][nc]]) and visited[nr][nc] > time:
            DFS(nr, nc, time+1)
...
for tc in range(1, T + 1):
    ...
    # 지도 정보
    tunnel = [list(map(int, input().split())) for _ in range(N)]
    visited = [[987654321] * M for _ in range(N)] # 시간체크 겸 방문 체크
